...
module.exports = {
    ...
    run: async () => {
        ...
        const bres = await mongoose.model('UserProfile').updateMany(
            {
                'recruiterEvents.0': { $exists: true },
                recruiterEvents: { $elemMatch: { $type: 'string' } },
            }, // array has at least one element and element type is string, not object
            [
                {
                    $set: {
                        recruiterEvents: {
